...
<body>
			...
			<table align="center">
				...
				<tr>
					...
					.getProperty("pasRaMasterRKAP.KodeKantorAuditorPemilik")%></label> : <input
						...
						id="KodeKantorAuditorPemilik" size="30" maxlength="30"
	...
	<!-- **********************TABLE RESULT************************************** -->
	<table id="dg" title="PASRAMASTERRKAP" class="easyui-datagrid"
		...
		<form id="detFm" method="post" novalidate>
			<table align="center">
				...
				<tr>
					<td><div class="fitem">
							<label><%=properties
					.getProperty("pasRaDetailRKAP.KodeMataAnggaran")%></label>
							:<input name="kodeMataAnggaran" class="easyui-textbox"
								id="kodeMataAnggaran"> <a href="javascript:void(0)"
								class="easyui-linkbutton c6" iconCls="icon-search"
								onclick="searchCOA()" style="width: 90px" id="btnCariCOA">Cari</a>
						</div></td>
				</tr>
				<tr>
					<td><div class="fitem">
							<label><%=properties
					.getProperty("pasRaDetailRKAP.NamaMataAnggaran")%></label>
				...
				<tr>
					<td><div class="fitem">
							<label><%=properties
					.getProperty("pasRaDetailRKAP.NominalMataAnggaran")%></label>
	...
	<div id="detDlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton c6"
			iconCls="icon-ok" onclick="detDoSave()" style="width: 120px"
			id="btnSaveDet">Save Detail</a> <a href="javascript:void(0)"
	...
	<div id="detDlg" class="easyui-dialog"
		...
		<form id="fmDM" method="post" novalidate>
			<table align="center">
				<tr>
					...
					<td><div class="fitem">
							<label><%=properties.getProperty("pasRaMasterRKAP.TahunRKAP")%></label>
							:<input name="tahunRKAPDM" class="easyui-textbox"
								id="tahunRKAPDM">
						</div></td>
				</tr>
		...
		<table id="detDg" title="LIST DETAIL RKAP" class="easyui-datagrid"
			...
			<thead>
				<tr>
					<th field="kodeMataAnggaran" width="100" sortable="true"><%=properties
					.getProperty("pasRaDetailRKAP.KodeMataAnggaran")%></th>
					<th field="namaMataAnggaran" width="100" sortable="true"><%=properties
					.getProperty("pasRaDetailRKAP.NamaMataAnggaran")%></th>
					...
					.getProperty("pasRaDetailRKAP.NominalMataAnggaran")%></th>
		...
		<form id="fmDMAju" method="post" novalidate>
			...
			<a href="javascript:void(0)" class="easyui-linkbutton c6"
				iconCls="icon-ok" onclick="pengajuan()" style="width: 90px"
				id="btnPengajuan">Ajukan</a> <a href="javascript:void(0)"
				class="easyui-linkbutton" iconCls="icon-cancel"
...
<script>
	...
	$("document")
			.ready(
					function() {
						...
						comboLookup($("#statusPembuatan"),
								'STATUS_PEMBUATAN_RKAP');
						comboLookup($("#statusReviu"), 'STATUS_REVIU_RKAP');
						comboLookup($("#statusPersetujuan"),
								'STATUS_PERSETUJUAN_RKAP');
	...
	function detRetrieve(kodeRKAP, nomorSuratPengajuan) {
		var row = $('#dg').datagrid('getSelected');
		...
		$('#kodeKantorAuditorPemilikDM').textbox('setValue',
				row.kodeKantorAuditorPemilik);
		...
		var jsonurl = 'pasRaDetailRKAPListAll.htm?' + 'KodeMataAnggaran=' + ""
				+ "&" + 'KodeRKAP=' + kodeRKAP + "&" + "userId=" + "${userId}";
		$('#detDg').datagrid({
			url : jsonurl,
	...
	function pengajuan() {
		...
		//simpan
		var url = 'pasRaMasterRKAPPengajuan.htm?' + "userId="
				+ "${userId}&kodeRKAPDM=" + $("#kodeRKAPDM").val();
		$('#fmDMAju').form('submit', {
			url : url,
			onSubmit : function() {
				return $(this).form('validate');
	...
	function searchCOA() {
		var coa = $('#kodeMataAnggaran').val();
		$.ajax({
			url : 'getRekIaMasterById.htm?param=' + coa,
			success : function(result) {
				if (result != 'null') {
					var x = JSON.parse(result);
					$('#namaMataAnggaran').textbox('setValue', x.description);
					$('#nominalMataAnggaran').textbox('readonly', false);
					$('#nominalMataAnggaran').textbox('textbox').focus();
					$('#btnSaveDet').linkbutton('enable');
				} else {
	...
	function retrieve() {
		var jsonurl = 'pasRaMasterRKAPListAll.htm?' + 'KodeRKAP='
				...
				+ 'KodeKantorAuditorPemilik='
				+ $('#KodeKantorAuditorPemilik').val() + "&"//StatusPembuatan StatusReviu
				+ 'StatusPersetujuan=' + "" + "&" + 'StatusPembuatan=' + ""
				+ "&" + 'StatusReviu=' + "" + "&" + "userId=" + "${userId}";
		$('#dg').datagrid({
			url : jsonurl,
			onLoadSuccess : function(data) {
				if (data.total == 0) {
	...
	function onAdd() {
		$('#kodeRKAP').textbox('readonly', true);
		$('#tahunRKAP').textbox('textbox').focus();
