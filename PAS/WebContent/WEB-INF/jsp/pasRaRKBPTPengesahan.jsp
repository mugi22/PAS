	...
	<table id="dg" title="PASRARKBPT" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th field="kodeKantorAuditorPemilik" width="100" sortable="true"><%=properties.getProperty("pasRaRKBPT.KodeKantorAuditorPemilik")%></th>
				...
				<th field="tanggalSuratPengajuan" width="100" sortable="true"><%=properties	.getProperty("pasRaRKBPT.TanggalSuratPengajuan")%></th>
				<th field="kodeUserPenyetuju" width="100" sortable="true"><%=properties.getProperty("pasRaRKBPT.KodeUserPenyetuju")%></th>
				<th field="nomorSuratPersetujuan" width="100" sortable="true"><%=properties.getProperty("pasRaRKBPT.NomorSuratPersetujuan")%></th>
				<th field="tanggalSuratPersetujuan" width="100" sortable="true"><%=properties.getProperty("pasRaRKBPT.TanggalSuratPersetujuan")%></th>
...
<!-- 
				...
				<th field="kodeRKBPTYangDiRevisi" width="100" sortable="true"><%=properties.getProperty("pasRaRKBPT.KodeRKBPTYangDiRevisi")%></th>
		...
		<form id="fm" method="post" novalidate>
			<table align="center">
				...
				<!-- 
                    ...
                    <tr><td><div class="fitem">	<label><%=properties.getProperty("pasRaRKBPT.TanggalSuratPengajuan")%></label> :<input name="tanggalSuratPengajuan"	class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" id="tanggalSuratPengajuan"></div></td></tr>	
                    ...
                    <tr><td><div class="fitem">	<label><%=properties.getProperty("pasRaRKBPT.NomorSuratPersetujuan")%></label> :<input name="nomorSuratPersetujuan"	class="easyui-textbox" id="nomorSuratPersetujuan"></div></td></tr>	
                    <tr><td><div class="fitem">	<label><%=properties.getProperty("pasRaRKBPT.TanggalSuratPersetujuan")%></label> :<input name="tanggalSuratPersetujuan"	class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" id="tanggalSuratPersetujuan"></div></td></tr>	
		...
		<form id="fmDMAju" method="post" novalidate>
			<table align="center">
				<tr>
					<td><div class="fitem">
							<label><%=properties
					.getProperty("pasRaRKBPT.NomorSuratPersetujuan")%></label>
							: <input name="nomorSuratPersetujuan" class="easyui-textbox"
								id="nomorSuratPersetujuanAju">
						</div></td>
					<td><div class="fitem">
							<label><%=properties
					.getProperty("pasRaRKBPT.TanggalSuratPersetujuan")%></label>
							: <input name="tanggalSuratPersetujuan" class="easyui-datebox"
								data-options="formatter:myformatter,parser:myparser"
								id="tanggalSuratPersetujuanAju">
						</div></td>
				</tr>
			</table>

			<a href="javascript:void(0)" class="easyui-linkbutton c6"
				iconCls="icon-ok" onclick="pengesahan()" style="width: 120px"
				id="btnPengajuan">Pengesahan</a> <a href="javascript:void(0)"
...
<script>
	...
	function retrieve() {
		var jsonurl = 'pasRaRKBPTListAll.htm?' + 'KodeKantorAuditorPemilik='
				...
				+ 'StatusPembuatan=SS' /*+ $('#StatusPembuatan').combobox('getValue')*/ + "&"//status pembuatan harus sudah selsai/SS
	...
	function doDelete() {
		...
		if (row.statusRKAP == 'A' || row.statusPembuatan == 'SS') {
			alert("Status RKAP " + row.statusRKAP + " atau "
	...
	function doCetak() {
		...
		window.open(repUrl + "&" + s,
						"_blank","toolbar=no, scrollbars=yes, resizable=yes,	directories=no, location=no, \
	...
	function doDetail() {
		...
		if (row == null) {
			...
			$//('#detKodeRKAP').textbox('setValue',row.kodeRKBPT);
			...
			onLoadSuccess : function(data) {
				...
				var x = statusPerstujuan;
				 if ( x == "S") {
					...
					$("#tanggalSuratPersetujuanAju").textbox('readonly',true);
					$("#nomorSuratPersetujuanAju").textbox('readonly',true);					
				}else{
					$("#tanggalSuratPersetujuanAju").textbox('readonly',false);
					$("#nomorSuratPersetujuanAju").textbox('readonly',false);	
	...
	function pengesahan() {
		...
		//simpan
		var url = 'pasRaRKBPTPengesahanSah.htm?' + "userId="
