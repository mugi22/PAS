...
<body>
	...
	<!-- **********************TABLE RESULT************************************** -->
	<table id="dg" title="PASRAMASTERRKAP PENGESAHAN" class="easyui-datagrid"
	...
	<div id="detDlgForm" class="easyui-dialog"
		...
		<form id="detFm" method="post" novalidate>
			...
			<tr>
				...
				<tr>
					<td><div class="fitem">
							<label><%=properties.getProperty("pasRaDetailRKAP.NominalMataAnggaranAprove")%></label>
							:<input name="nominalMataAnggaranAprove" class="easyui-numberbox"
								data-options="min:0,precision:0,groupSeparator:','"
								id="nominalMataAnggaranAprove">
						</div></td>
				</tr>
			</table>
		</form>
	...
	<table id="detDg" title="LIST DETAIL RKAP" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th field="nominalMataAnggaranAprove" width="100" sortable="true"
					data-options="formatter:function(value, row){return accounting.formatNumber(row.nominalMataAnggaranAprove,0,'.',','); }"
					align="right"><%=properties.getProperty("pasRaDetailRKAP.NominalMataAnggaranAprove")%></th>		
		...
		<form id="fmDMAju" method="post" novalidate>
			<table align="center">
				<tr>
					...
					.getProperty("pasRaMasterRKAP.NomorSuratPersetujuan")%></label> :<input
						name="nomorSuratPersetujuan" class="easyui-textbox"
						id="nomorSuratPersetujuan">
						</div></td>
					<td><div class="fitem">
							<label><%=properties
					.getProperty("pasRaMasterRKAP.TanggalSuratPersetujuan")%></label> :<input
								name="tanggalSuratPersetujuan" class="easyui-datebox"
								data-options="formatter:myformatter,parser:myparser"
								id="tanggalSuratPersetujuan">
						</div></td>
				</tr>
...
<script>
	...
	function detDoEdit() {
		...
		if (row) {
			...
			$('#detFm').form('clear');
			$('#detFm').form('load', row);
			url = 'pasRaDetailRKAPSahEdit.htm?'+"userId="+"${userId}";//?param='+row.kodeProvinsi+'&param2='+row.kodeKabupaten; //SESUAIKAN
			onEditDet();
		}
	}
	function detDoShow() {
	...
	function pengesahan(){
		...
		//simpan
		var url = 'pasRaMasterRKAPPengeSah.htm?' + "userId=" + "${userId}&kodeRKAPDM="+$("#kodeRKAPDM").val();
		$('#fmDMAju').form('submit', {
			url : url,
			onSubmit : function() {
				return $(this).form('validate');
