...
<body>
	...
	<div id="toolbar">
		...
		<td align="right"><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-authorize" plain="true"
			onclick="doDetail()" id="btnDetail"><%="Show "+properties.getProperty("button.detail")%></a>
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-authorize" plain="true"
			onclick="doPersetujuan()" id="btnPersetujuan">Persetujuan</a>
		</td>
...
<script>
	...
	function detDoEdit() {
		...
		if (row) {
			...
			comboEaJenisPemeriksaan($('#detkodeJenisPemeriksaan'),
					row.kodeJenisPemeriksaan);
			$.ajax({
				url : "getPasEaAuditan.htm?param=" + $("#detkodeAuditan").val()
						+ "&param2=",
		...
		$('#detkodeJenisPemeriksaan').combobox(
				{
					onSelect : function(rec) {
						$('#detnamaKegiatan').textbox(
								'setValue',
								"Pemeriksaan "
										+ $('#detkodeJenisPemeriksaan')
												.combobox('getText') + " = "
										+ $("#detnamaAuditan").val());
					}
				});
	...
	function persetujuan() {
		...
		if (row) {
			...
			$('#fm').form('clear');
			$('#fm').form('load', row);
			url = 'pasRaPKPTPersetujuan.htm?' + "userId=" + "${userId}";//?param='+row.kodeProvinsi+'&param2='+row.kodeKabupaten; //SESUAIKAN
			onEdit();
		}
	}
	...
	function doPersetujuan(){
		var row = $('#dg').datagrid('getSelected');
		//alert(row.statusPerstujuan);
		if(row.statusPKPT != "P"){
			alert("Bukan Status Persetujuan");
			return;
		}
		if(row.statusPerstujuan == 'Y'){
			persetujuan();
			onShow();
